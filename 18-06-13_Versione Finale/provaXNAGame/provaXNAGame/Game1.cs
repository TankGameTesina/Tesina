                    ...
                    foreach (BulletObject b in bullets)
                    {
                        ...
                        if (result != -2)
                        {
                            ...
                            else
                            {
                                if (life <= 0)
                                {
                                    if (playersInGame.Count <= 2 && partitaValida)
                                    {
                                        foreach (GameObject tnk in players)
                                        {
                                            msg = result + ";die"; //id;die
                                            Send(new Pacchetto(msg, tnk.Address));
                                            msg = playersInGame[0].Id + ";win";
                                            Send(new Pacchetto(msg, tnk.Address));
                                        }
                                    }
                                    else
            ...
            switch (ann[0])
            {
                ...
                case "srv": //messaggi dal server
                    switch (ann[1])
                    {
                        ...
                        case "ref":
                            MessageBox.Show("Sei gia connesso al server, attendere la fine della partita!");
                            clear();
                ...
                case "r":
                    if (serverStatus)
                    {
                        ...
                        foreach (GameObject player in players)
                            if (pkt.Address == player.Address)
                                playerDie = true;

                        if (!playerDie && playersInGame.Count < postiMax && (playersInGame.FirstOrDefault(x => x.Username == ann[1] && x.Address == pkt.Address)) == null) //ann[1] username
                        {
                            app = new GameObject();
                            playersInGame.Add(app);
                            app.InitEnemy(PosizioneInizialeCarri(), Content.Load<Texture2D>("tankEnemy"), ann[1], pkt.Address, playersInGame.IndexOf(app));
                            messages.Add(new Messaggio(ann[1] + " si e' unito", Color.Blue));
                            Send(new Pacchetto("srv;acc;" + app.Id + ";" + larghezzaSchermoServer + ";" + altezzaSchermoServer + ";", pkt.Address));
                ...
                case "dct":
                    if (serverStatus)
                    {
                        ...
                        if (app != null)
                        {
                            ...
                            foreach (GameObject tnk in playersInGame.Where(x => x.Id != myId))
                            {
                ...
                case "user":
                    app = new GameObject();
                    playersInGame.Add(app);
                    app.InitEnemy(new Vector2(100, 400), Content.Load<Texture2D>("tankEnemy"), ann[2], pkt.Address, Convert.ToInt32(ann[1]));
                ...
                default:
                    ...
                    if (Int32.TryParse(ann[0], out num))
                    {
                        ...
                        if (playersInGame.Contains(app))
                        {
                            if (ann[1] == "die")
                            {
                                messages.Add(new Messaggio(app.Username + " e' stato distrutto!", Color.Orange));
